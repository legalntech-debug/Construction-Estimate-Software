...
export default function AccessControlPage() {
  ...
  const fetchAccessControlData = async () => {
    ...
    try {
      ...
      // 2. Fetch all profiles for RBAC table
      const profilesRes = await supabase.rpc('get_profiles_with_estimates_count');
      let sortedProfiles = profilesRes.data || [];
  ...
  return (
    <div className="p-4 sm:p-8 bg-slate-50/50 min-h-screen space-y-8 font-sans antialiased text-slate-950 relative select-none">
      ...
      {/* RBAC Table & Role Management Panel */}
      <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h3 className="font-bold text-slate-800 text-base">User Privilege & Account Security (RBAC)</h3>
            <p className="text-xs text-slate-500">View live lock statuses and toggle administrative access or account suspensions.</p>
          </div>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredProfiles.length > 0 ? (
                    ...
                    <tr key={userId} className="hover:bg-slate-50/50 transition">
                      ...
                      <td className="p-3 text-center">
                        {isAdminUser ? (
                          ...
                          <span className="px-2.5 py-1 bg-indigo-50 text-indigo-700 rounded-lg text-[10px] font-extrabold uppercase">Exempt (Marketing & Support)</span>
                        ) : isInvestor ? (
                          <span className="px-2.5 py-1 bg-teal-50 text-teal-700 rounded-lg text-[10px] font-extrabold uppercase">Exempt (Investor)</span>
                        ) : isPremium ? (
                ...
                <tr>
                  <td colSpan={7} className="p-6 text-center text-slate-400 text-sm">No profiles match your search criteria.</td>
                </tr>
              )}
            </tbody>
          </table>
