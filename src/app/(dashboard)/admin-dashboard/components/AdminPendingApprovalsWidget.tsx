...
export default function AdminPendingApprovalsWidget({ onActionComplete }: { onActionComplete: () => void }) {
  ...
  return (
    <div className="bg-white p-6 rounded-3xl border border-amber-200 shadow-sm space-y-4 my-6">
      <div className="flex justify-between items-center">
        <div>
          <h3 className="font-bold text-amber-800 text-base">⏳ New User Registration Requests</h3>
          <p className="text-xs text-slate-500">Approve new users to grant them access to the platform and features.</p>
        </div>
        <span className="px-3 py-1 bg-amber-100 text-amber-800 rounded-xl font-bold text-xs">
              ...
              <tr key={req.id} className="hover:bg-slate-50/50 transition text-xs">
                ...
                <td className="p-3 text-right space-x-2">
                  <button
                    ...
                    className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl transition shadow-sm uppercase text-[10px]"
                  >
                    Approve
                  </button>
                  <button
                    onClick={() => handleReject(req.id, req.email)}
                    className="px-3 py-1.5 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-xl transition shadow-sm uppercase text-[10px]"
                  >
                    Reject
                  </button>
                </td>
