...
import { VastuAssessment, VastuDirection } from "./planningTypes";
...
export function assessVastuForRoom(
  ...
  switch (roomKey) {
    ...
    case "STAIRCASE":
      if (assignedZone === "S" || assignedZone === "W" || assignedZone === "SW") {
        return { zone: assignedZone, status: "GOOD", note: "Heavy zones (South, West, South-West) are optimal for stairs." };
      }
      return { zone: assignedZone, status: "ALTERNATIVE ZONE", note: "Southern (S/SW) or Western (W/SW) quadrants preferred for staircases." };
